...
if (isset($_GET['delete'])) {
    ...
    header("location:?page=sliders&hapus=berhasil");
}
...
if (isset($_POST['simpan'])) {
    ...
    if ($id) {
        // ini query update
        $update = mysqli_query($koneksi, "UPDATE sliders SET tittle = '$tittle', description = '$description', image = '$image_name' WHERE id = '$id'");
        if ($update) {
            header("location:?page=sliders&ubah=berhasil");
        }
    } else {
        $insert = mysqli_query($koneksi, "INSERT INTO sliders (tittle, description, image) VALUES ('$tittle', '$description', '$image_name')");
        if ($insert) {
            header("location:?page=sliders&tambah=berhasil");
        }
    }
}
...
<section class="section">
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="card">
                <div class="card-body">
                    <h5 class="card-tittle "><?php echo $tittle; ?></h5>
                    <form action="" method="post" enctype="multipart/form-data">

                        <div class="mb-3">
                            <label for="tittle" class="form-label">tittle</label>
                            <input type="text" class="form-control" id="tittle" name="tittle" placeholder="Masukkan tittle Anda" value="<?php echo ($id) ? $rowEdit['tittle'] : '' ?>" required>
                        </div>
                        <div class="mb-3">
